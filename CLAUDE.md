...
# Run the server in dev mode: no authentication, and the public default key
# is allowed (--dev-mode implies --allow-public-key)
...
- **AuditEvent retention**: none by default. A deployment that sets `auditRetention` (CLI `--audit-retention-days`) has the hourly sweep remove older AuditEvents with their history and index rows, through fhir_db's `purgeResourcesLastUpdatedBefore`. No source sets a period for audit records (NIST SP 800-66r2 §5.3.2 leaves it to the risk assessment), so the server deletes nothing unless told (REVIEW-2026-09-17 A16)
...
- **Android**: foreground service keeps server running when backgrounded. When the phone kills the app, Android restarts only the service; `serverTaskCallback` (`lib/src/services/server_task.dart`) then starts the server if the user left it on (`server_wanted` in SharedPreferences, set by Start, cleared by Stop). Opening the app adopts a server already answering instead of starting a second. Start asks for notification permission (verified on a OnePlus: once granted, the service's "FHIR ANT Server" notice was posted; before, it was hidden) and, when fhirant is not exempt from battery optimisation, shows `keep_running_dialog.dart`: a button to the battery settings list (never the direct request: Play rejected Syncthing for REQUEST_IGNORE_BATTERY_OPTIMIZATIONS, syncthing-android issue 1039) and the steps to lock the app in Recents. Measured on a OnePlus: `tool/background_survival/RESULTS.md`
- **iOS**: server stops when app goes to background (OS limitation)

## Testing

**1,477 tests** across 138 test files, all passing (counted 2026-09-14; server recounted 2026-09-22).

- **Server tests** (1,315 tests, 126 files): `cd packages/fhirant_server && dart test`
...
--dev-mode          Disable authentication; implies --allow-public-key
--allow-public-key  Open the store under the public default key (throwaway
                    databases); alone, it keeps authentication on
--spec-path         FHIR spec NDJSON directory (default /app/fhir_spec)
--base-url          The URL clients reach this server by
--audit-retention-days  Days AuditEvents are kept (unset: none are removed)
```
