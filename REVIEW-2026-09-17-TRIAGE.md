# REVIEW-2026-09-17 triage: 4 OPEN (three structural, one from the differential run)
...
| ST5 | ~~`fhirant_secure_storage` half dead~~ | **DONE 2026-09-23 (Grey: delete).** Measured first: of 13 methods, 3 are called by the app and server, 4 more by the package's own certificate code, and 6 by nothing (passkey store/read, encryption-key save/read, two random-key generators). The 6, their two unused key-name constants and the one test group that exercised only them are deleted; the app keeps its own two secrets under its own names, so nothing stored on a phone changes | | |
