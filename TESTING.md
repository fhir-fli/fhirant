...
| File | What it covers |
|------|----------------|
| `test/cli/options_test.dart` | REVIEW-2026-09-17 S6: defaults; the `--config` YAML file sets what the command line did not and the command line wins; an unknown key, a non-map or unparsable file, a port outside 1–65535 and a non-positive retention are usage errors; S4: `--dev-mode` implies `--allow-public-key` and, with authentication on, the public key needs its own flag (13) |
...
| `test/integration/every_canonical_lookup_honours_version_test.dart` | Two versions of one ValueSet and one CodeSystem held: `$expand?url=X|2` expands version 2, `$lookup?system=X|1` reads version 1, and `url|3` answers 404; passed before the shared finder and pins what it keeps (3) |
| `test/integration/a_degraded_health_check_logs_why_test.dart` | A health check whose store probe fails answers `degraded` and logs a warning carrying the exception; it logged nothing before (1) |
