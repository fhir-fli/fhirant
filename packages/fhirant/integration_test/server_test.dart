...
void main() {
  ...
  group('Server endpoints (on-device)', () {
    ...
    test('CRUD lifecycle works', () async {
      // Create
      final (createStatus, created) = await request(
        'POST',
        '/Patient',
        body: {
          'resourceType': 'Patient',
    ...
    test('backup and restore work', () async {
      ...
      // Backup
      final (backupStatus, backup) = await request('POST', r'/$backup');
      ...
      final (restoreStatus, restoreResult) = await request(
        'POST',
        r'/$restore',
        body: {
          'resourceType': 'Bundle',
