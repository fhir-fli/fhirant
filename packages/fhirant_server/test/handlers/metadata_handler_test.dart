...
void main() {
  group('metadataHandler', () {
    ...
    test('software and implementation present', () {
      ...
      expect(cs.implementation!.description.valueString, contains('FHIRant'));
    });
    ...
    test('system-level interactions include search-system', () {
      final codes = rest.interaction!.map((i) => i.code.toString()).toList();
    ...
    test(
        'system-level operations include validate, fhirpath, transform, export',
        () {
      final opNames = rest.operation!.map((o) => o.name.valueString).toList();
    ...
    test(r'$everything on Patient but not on Observation', () {
      ...
      final obs =
          rest.resource!.firstWhere((r) => r.type.valueString == 'Observation');
      final obsOps = obs.operation!.map((o) => o.name.valueString).toList();
    ...
    test(r'$export on Patient and Group, not on Observation', () {
      ...
      final group =
          rest.resource!.firstWhere((r) => r.type.valueString == 'Group');
      final groupOps = group.operation!.map((o) => o.name.valueString).toList();
    ...
    test('all resource types are present', () {
      final types = rest.resource!.map((r) => r.type.valueString).toSet();
      // Spot check
