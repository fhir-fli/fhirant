...
Future<void> main(List<String> args) async {
  ...
  Future<void> timed(
    ...
    out.writeAsStringSync(
      '$round\t$step\t${sw.elapsedMilliseconds}\n',
  ...
  for (var round = 1; round <= 3; round++) {
    ...
    await timed(round, 'SQL id page (covering index)', () async {
      await db
          .customSelect(
            "SELECT DISTINCT id FROM reference_search_parameters WHERE resource_type = 'Observation' AND search_name = 'subject' AND reference_resource_type = 'Patient' AND reference_id_part = '$big' ORDER BY id LIMIT 20",
          )
          .get();
    });
    ...
    await timed(round, 'SQL 20 rows IN (...)', () async {
      ...
      await db.customSelect(
        "SELECT resource FROM resources WHERE resource_type = 'Observation' AND id IN ($ph)",
