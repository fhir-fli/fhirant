...
Future<void> createChargeItemTables(Transaction txn) async {
  await txn.execute('''
...
Future<bool> saveChargeItem(Transaction txn, ChargeItem resource) async {
  ...
  try {
    final existingResource = await txn.rawQuery(
      'SELECT id, resource, lastUpdated FROM ChargeItem WHERE id = ?',
      [id],
    );
    ...
    await txn.rawInsert(
      '''
      INSERT OR REPLACE INTO ChargeItem (
        id, lastUpdated, resource
      ) VALUES (?, ?, ?)
      ''',
      [
...
Future<ChargeItem?> getChargeItem(Transaction txn, String id) async {
  try {
