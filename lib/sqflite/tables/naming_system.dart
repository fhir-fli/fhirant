...
Future<void> createNamingSystemTables(Transaction txn) async {
  await txn.execute('''
...
Future<bool> saveNamingSystem(Transaction txn, NamingSystem resource) async {
  ...
  try {
    final existingResource = await txn.rawQuery(
      'SELECT id, resource, lastUpdated FROM NamingSystem WHERE id = ?',
      [id],
    );
    ...
    await txn.rawInsert(
      '''
      INSERT OR REPLACE INTO NamingSystem (
        id, lastUpdated, resource
      ) VALUES (?, ?, ?)
      ''',
      [
...
Future<NamingSystem?> getNamingSystem(Transaction txn, String id) async {
  try {
