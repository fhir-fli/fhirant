...
void createMolecularSequenceTables(Database db) {
  db
    ..execute('''
...
bool saveMolecularSequence(Database db, MolecularSequence resource) {
  ...
  try {
    ...
    if (db.select(
      'SELECT id FROM MolecularSequence WHERE id = ?',
      [id],
    ).isNotEmpty) {
      db.execute(
        '''
        ...
        ) SELECT id, lastUpdated, resource FROM MolecularSequence WHERE id = ?;
      ''',
        [id],
      );
    }
