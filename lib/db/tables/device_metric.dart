...
bool saveDeviceMetric(Database db, DeviceMetric resource) {
  final updatedResource = updateMeta(resource, versionIdAsTime: true).newIdIfNoId() as DeviceMetric;
  ...
  try {
    ...
    if (db.select('SELECT id FROM DeviceMetric WHERE id = ?', [id]).isNotEmpty) {
      db.execute('''
...
DeviceMetric? getDeviceMetric(Database db, String id) {
  try {
    final result = db.select('SELECT resource FROM DeviceMetric WHERE id = ?', [id]);
