...
bool saveResearchSubject(Database db, ResearchSubject resource) {
  final updatedResource = updateMeta(resource, versionIdAsTime: true).newIdIfNoId() as ResearchSubject;
  ...
  try {
    ...
    if (db.select('SELECT id FROM ResearchSubject WHERE id = ?', [id]).isNotEmpty) {
      db.execute('''
...
ResearchSubject? getResearchSubject(Database db, String id) {
  try {
    final result = db.select('SELECT resource FROM ResearchSubject WHERE id = ?', [id]);
